(function () {
    const MODEL_IDS = [4, 5, 6];
    const COLORS = { 4: "#4a8fa8", 5: "#c4732d", 6: "#8c4009" };
    const GREY = "#c0bdb9";
    const FONT = "Hanken Grotesk, sans-serif";
    const MAX_EPOCH = 25000;

    // ── Shared interaction state ──────────────────────────────────────────────
    let selectedModel = null;
    let hoveredModelId = null;
    let hoveredEpoch = null;   // actual epoch value, not an index
    ...
    function initCharts() {
        ...
        Promise.all([
            d3.csv("data/gd_metadata.csv"),
            d3.csv("data/gd_output.csv")
            ...
            output.forEach(row => {
                ...
                const ep = +row.epoch;
                const loss = row.train_loss.trim() !== "" ? +row.train_loss : null;
                const sh = row.sharpness_H.trim() !== "" ? +row.sharpness_H : null;
                // Only push epoch once; track all fields per epoch
            ...
            const MODELS = MODEL_IDS.map(id => ({
                id, lr: lrMap[id],
                label: "η = " + lrMap[id].toFixed(2),
                color: COLORS[id],
                thresh: 2 / lrMap[id],
                series: {
                    // Loss: downsample to every 25th point (~1000 pts) for performance
                    lossEpoch: seriesMap[id].epoch.filter((_, i) => i % 25 === 0),
                    loss: seriesMap[id].loss.filter((_, i) => i % 25 === 0),
                    // Sharpness: keep only the ~100 defined samples, pre-filtered
                    sharpEpoch: seriesMap[id].epoch.filter((_, i) => seriesMap[id].sharp[i] != null),
                    sharp: seriesMap[id].sharp.filter(v => v != null),
                }
            }));

            [
                { containerId: "gd-loss-chart", title: "Train Loss", yKey: "loss", yLabel: "MSE Loss", logScale: true, yDomain: [0.001, 3], thresholds: false, chartIdx: 0 },
                { containerId: "gd-sharp-chart", title: "Sharpness", yKey: "sharp", yLabel: "Sharpness", logScale: false, yDomain: [0, 90], thresholds: true, chartIdx: 1 },
            ].forEach(cfg => drawChart({ ...cfg, models: MODELS }));

        }).catch(() => { });
    }

    // ── Chart drawing ─────────────────────────────────────────────────────────
    function drawChart({ containerId, title, yKey, yLabel, yDomain, logScale, thresholds, models, chartIdx }) {
        ...
        const xScale = d3.scaleLinear().domain([1, MAX_EPOCH]).range([0, iw]);
        const yScale = logScale
            ? d3.scaleLog().domain(yDomain).range([ih, 0]).clamp(true)
            : d3.scaleLinear().domain(yDomain).range([ih, 0]).clamp(true);

        // X-axis ticks: 0, 5k, 10k, 15k, 20k, 25k
        const xTickFmt = v => v === 0 ? "0" : (v / 1000) + "k";

        // Grid
        g.append("g")
            .call(d3.axisLeft(yScale).ticks(logScale ? 4 : 5).tickSize(-iw).tickFormat(""))
        ...
        // Y axis
        const yTickFmt = logScale
            ? v => v < 0.01 ? d3.format(".0e")(v) : d3.format(".2~f")(v)
            : null;
        g.append("g")
            .call(d3.axisLeft(yScale).ticks(logScale ? 4 : 5, yTickFmt))
            .call(ax => ax.select(".domain").attr("stroke", "#d4ccc5"))
            .call(ax => ax.selectAll("text").attr("font-family", FONT).attr("font-size", "10px").attr("fill", "#777"))
            .call(ax => ax.selectAll("line").attr("stroke", "#d4ccc5"));
        ...
        // Lines — use pre-filtered pts for each key
        ...
        g.append("rect")
            ...
            .on("mousemove", function (event) {
                ...
                hoveredEpoch = epochs[idx];   // store value, not index
                ...
                visibleModels.forEach(m => {
                    ...
                    const dist = Math.abs(yScale(val) - my);
                    if (dist < closestDist) { closestDist = dist; closestId = m.id; }
                });
        ...
        function update() {
            ...
            if (hoveredEpoch !== null) {
                // Resolve the nearest local index for this chart's epoch array
